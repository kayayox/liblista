 ...
 * @brief Ejemplo de uso de liblista con structs complejos
 */
...
int main(){
    ...
    resultado=lista_crear_avanzado(&lista,sizeof(Persona),copiar_persona,liberar_persona,comparar_persona);
    if(resultado!=EXITO){
    ...
    for(int i=0;i<3;i++){
        resultado=lista_insertar_final(lista,&personas[i]);
        if(resultado!=EXITO){
    ...
    do{
        ...
        if(tiene_siguiente){
            iterador_siguiente(iterador,&persona_actual);
    ...
    resultado=lista_eliminar_valor(lista,&persona_a_eliminar);
    if(resultado==EXITO){
